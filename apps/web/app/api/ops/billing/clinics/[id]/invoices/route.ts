...
export async function GET(req: Request, ctx: Ctx) {
  ...
  const upstream = await callOpsApi(req, `/api/internal/billing/clinics/${clinicId}/invoices`, { method: "GET" });
  const json = (await upstream.json().catch(() => null)) as any;
  if (!upstream.ok || !json || json.ok !== true) {
    return NextResponse.json(fail(String(json?.error || "upstream_error"), "Upstream invoices failed", { status: upstream.status }), {
