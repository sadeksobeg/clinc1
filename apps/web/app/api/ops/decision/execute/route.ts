...
import { executeDecisionAction } from "@/lib/ops-server";
...
export async function POST(req: Request) {
  ...
  const payload = {
    ...
    action_id: String((body as { action_id?: string }).action_id ?? ""),
    decision: String((body as { decision?: string }).decision ?? "reject") as "confirm" | "reject",
  };
  ...
  if (!payload.action_id || payload.action_id.length < 3) {
    return NextResponse.json({ ok: false, error: "bad_action_id" }, { status: 400 });
  }
  if (payload.decision !== "confirm" && payload.decision !== "reject") {
    return NextResponse.json({ ok: false, error: "bad_decision" }, { status: 400 });
  }

  const out = await executeDecisionAction(payload);
  return NextResponse.json(out, { status: out.ok ? 200 : out.code === "overlap" ? 409 : 400 });
}
