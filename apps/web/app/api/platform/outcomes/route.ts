...
export async function GET(req: Request) {
  ...
  const upstream = await callOpsApi(req, `/api/internal/platform/outcomes${qs ? `?${qs}` : ""}`, { method: "GET" });
  const json = (await upstream.json().catch(() => null)) as any;
  if (!upstream.ok || !json || json.ok !== true) {
    return NextResponse.json(fail(String(json?.error || "upstream_error"), "Upstream outcomes failed", { status: upstream.status }), {
