...
export async function POST(req: Request, ctx: Ctx) {
  ...
  const upstream = await callOpsApi(req, `/api/internal/platform/incidents/${id}/assign`, { method: "POST", bodyObject: body });
  const json = (await upstream.json().catch(() => null)) as any;
  if (!upstream.ok || !json || json.ok !== true) {
    return NextResponse.json(fail(String(json?.error || "upstream_error"), "Upstream assign failed", { status: upstream.status }), {
